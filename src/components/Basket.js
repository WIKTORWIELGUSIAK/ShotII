...
export default function Basket(props) {
  ...
  return (
    <div
      className={`py-16 flex flex-col w-full bg-gray-100 w-full min-h-screen`}
    >
      <h2 className="mt-10 w-full mx-auto text-center text-6xl font-semibold">
        Cart Items
      </h2>
      {cartItems.length === 0 && (
      ...
      {cartItems.length !== 0 && (
        <div className="mt-10">
          <div className="py-2">ITEMS PRICE: {itemPrice.toFixed(2)}$</div>
          <div className="py-2">
            SHIPPING PRICE: {shippingPrice.toFixed(2)}$
          </div>
          <div className="py-2">TOTAL PRICE: {totalPrice.toFixed(2)}$</div>
        </div>
      )}
    </div>
  );
